...
## 새 게임 추가 방법 A — 관리 페이지 (추천)

사이트의 `/admin.html`(포털 푸터의 "관리" 링크)에서 브라우저로 바로 올릴 수 있습니다.

1. 최초 1회: GitHub Fine-grained 토큰 발급 (관리 페이지 안에 단계별 안내 있음)
   - Repository access: `aigameportal`만, Permissions: Contents → Read and write
2. "새 게임 추가" → 메타데이터 입력 + 게임 폴더/썸네일/배너 이미지 선택 → 저장하고 배포
   - 썸네일은 640×400, 배너는 1280×640으로 자동 크롭
   - games.json 갱신까지 한 커밋으로 처리되고, 커밋되면 자동 배포
- 토큰은 브라우저(localStorage)에만 저장되고 저장소에는 절대 올라가지 않습니다.
